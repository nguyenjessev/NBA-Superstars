...
function simGame(team1, team2) {
    ...
    function doShot() {
        if(Math.floor(Math.random()*101) < threePointFrequency+playerPossession.tendencies["3pt"]) {
            ...
            var matchup = otherTeam.players[Math.floor(Math.random()*5)];
            if(Math.floor(Math.random() * 100) > (playerPossession.stats["3pt Shooting"] - matchup.stats["Blocking"] + 25) && Math.random() < 0.15) {
            ...
            var matchup = otherTeam.players[Math.floor(Math.random()*5)];
            if(Math.floor(Math.random() * 100) > (playerPossession.stats["Shooting"] - matchup.stats["Blocking"] + 25) && Math.random() < 0.15) {
                matchup.gameStats["BLK"]++;
